...
lemma uppercase_char_print_empty[print_empty, fp_NER]:
  "p_has_result (print uppercase_char) i [] \<longleftrightarrow> False"
  by (clarsimp simp add: uppercase_char_def print_empty)



\<comment> \<open>PNGI, PASI\<close>
lemma uppercase_char_PNGI[PASI_PNGI, PASI_PNGI_intros]:
  "PNGI (parse uppercase_char)"
  unfolding uppercase_char_def
  by (rule any_from_set_PNGI)

lemma uppercase_char_PASI[PASI_PNGI, PASI_PNGI_intros]:
...
lemma uppercase_char_fpci[fpci_simps]:
